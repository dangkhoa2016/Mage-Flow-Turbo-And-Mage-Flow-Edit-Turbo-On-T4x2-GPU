...
- `scripts/run_public_candidate.sh` xác minh runtime, model mount, source commit, token handling và topology T4 x2.
...
- Candidate đã được acceptance ghi nhận resident reuse, không re-extract runtime và không sửa vendor source.
...
Source-level corrective closeout đã hoàn tất; công việc còn lại chỉ liên quan publication và không yêu cầu GPU re-qualification.
