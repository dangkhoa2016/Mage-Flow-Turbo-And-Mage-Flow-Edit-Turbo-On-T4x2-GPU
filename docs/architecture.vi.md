...
`scripts/run_public_candidate.sh` khởi động hoặc tái sử dụng T2I/Edit worker đang healthy. Live acceptance không reload model. Candidate đã được acceptance ghi nhận resident-worker reuse và không re-extract runtime trong reuse path.
